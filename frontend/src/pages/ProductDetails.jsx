...
function ProductDetails() {
       ...
       const {loading:cartLoading,error:cartError,success,message,cartItems: _cartItems}=useSelector((state)=>state.cart);
       ...
       const increaseQuantity=()=>{
        if(product.stock<=quantity){
            console.error('Cannot exceed available Stock!')
            dispatch(removeErrors())
            return;
        }
        setQuantity(qty=>qty+1)
    ...
    <div className="product-details-container">
        <div className="product-detail-container">
            <div className="product-image-container">
                ...
                {product.image && product.image.length>1 && (<div className="product-thumbnails">
            ...
            { product.stock>0 && (<>  <div className="quantity-controls">
                ...
                {cartError && (
                  <div style={{ background: '#fdecea', color: '#611a15', border: '1px solid #f5c6cb', borderRadius: '8px', padding: '10px 12px', marginBottom: '12px' }}>
                    {typeof cartError === 'string' ? cartError : (cartError.message || 'An error occurred')}
                  </div>
                )}
                {success && message && (
