...
export const getProduct=createAsyncThunk('product/getProduct',async({keyword,page=1,category,minRating,priceMin,priceMax,sort},{rejectWithValue})=>{
    try{
        ...
        if(minRating!==undefined && minRating!==null){
            link+=`&minRating=${minRating}`;
        }
        if(priceMin!==undefined && priceMin!==null){
            link+=`&priceMin=${priceMin}`;
        }
        if(priceMax!==undefined && priceMax!==null){
            link+=`&priceMax=${priceMax}`;
        }
